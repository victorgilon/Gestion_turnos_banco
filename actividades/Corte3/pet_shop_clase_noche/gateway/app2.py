...
def resumen():
    ...
    global circuito_abierto_mascotas, circuito_abierto_usuarios

    # Circuit Breaker abierto
    if circuito_abierto_usuarios or circuito_abierto_mascotas:
        print("El circuito de resumen esta abierto", flush=True)
        return jsonify({"error": "Resumen no disponible. Servicio dependiente bloqueado",
                        "detalles":{
                            "usuarios": "Bloqueado" if circuito_abierto_usuarios else "Disponible",
                            "mascotas": "Bloqueado" if circuito_abierto_mascotas else "Disponible"
                        }}), 503

    try:
        # Peticiones a los servicios
        print("Consultando usuarios...", flush=True)
        usuarios_data = requests.get("http://usuarios:5000/usuarios", timeout=2).json()

        print("Consultando mascotas...", flush=True)
        mascotas_data = requests.get("http://backend:5000/mascotas", timeout=4).json()

        # Reiniciar fallos si todo funciona
        fallos_resumen = 0
        ...
        print("Error al obtener datos del resumen", flush=True)
        # Manejamos el caso en que fallen durante la ejecución del resumen
        return jsonify({"error": "Servicio de resumen no disponible en este momento"}), 503
